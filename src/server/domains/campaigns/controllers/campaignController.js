...
class CampaignController {
    constructor() {
        ...
        this.dbCampaignStorageService = dbCampaignStorageService;
    }
    ...
    async getAllCampaigns(req, res) {
        try {
            ...
            // If using database storage, also get campaigns from SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (useDatabaseStorage) {
                const dbCampaigns = await this.dbCampaignStorageService.listCampaigns();
                // Add database campaigns to the response
                for (const dbCampaignId of dbCampaigns) {
                    const dbCampaign = await this.dbCampaignStorageService.loadCampaign(dbCampaignId);
                    if (dbCampaign) {
                        // Check if this campaign is already in the in-memory list to avoid duplicates
                        const existingIndex = campaignsJson.findIndex(c => c.id === dbCampaignId);
                        if (existingIndex === -1) {
                            campaignsJson.push({
                                ...dbCampaign,
                                id: dbCampaignId,
                            ...
                            if (dbCampaign.updated_at && new Date(dbCampaign.updated_at) > new Date(campaignsJson[existingIndex].updated_at || campaignsJson[existingIndex].createdAt || 0)) {
                                campaignsJson[existingIndex] = {
                                    ...dbCampaign,
                                    id: dbCampaignId,
                                    source: 'database'
                                };
            ...
            logger.info(`Retrieved ${campaignsJson.length} campaigns`);
    ...
    async getCampaign(req, res) {
        try {
            ...
            let campaign = this.campaignManager.getCampaign(campaignId);
            
            // If not found in memory and using database storage, check SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (!campaign && useDatabaseStorage) {
                const dbCampaign = await this.dbCampaignStorageService.loadCampaign(campaignId);
                if (dbCampaign) {
                    campaign = {
                        ...dbCampaign,
                        id: campaignId,
    ...
    async createCampaign(req, res) {
        try {
            ...
            if (useDatabaseStorage) {
                try {
                    await this.dbCampaignStorageService.saveCampaign(campaign.id, campaign.toJSON ? campaign.toJSON() : campaign);
                    logger.info('Campaign saved to database', { campaignId: campaign.id });
                } catch (dbError) {
    ...
    async updateCampaign(req, res) {
        try {
            ...
            let campaign = await this.campaignManager.updateCampaign(campaignId, updateData);
            
            // If using database storage, also update SQLite
            const useDatabaseStorage = req.body?.useDatabaseStorage === true;
            if (useDatabaseStorage) {
                try {
                    if (!campaign) {
                        // If campaign doesn't exist in memory, try to load from database
                        campaign = await this.dbCampaignStorageService.loadCampaign(campaignId);
                    }
                    ...
                    if (campaign) {
                        ...
                        await this.dbCampaignStorageService.saveCampaign(campaignId, updatedCampaign);
                        logger.info('Campaign updated in database', { campaignId: campaignId });
                    }
                } catch (dbError) {
                    logger.error('Failed to update campaign in database', { 
                        error: dbError.message, 
                        campaignId: campaignId 
                    });
                    // Don't fail the entire request just because the database update failed
                }
            }
    ...
    async deleteCampaign(req, res) {
        try {
            ...
            if (useDatabaseStorage) {
                try {
                    await this.dbCampaignStorageService.deleteCampaign(campaignId);
                    logger.info('Campaign deleted from database', { campaignId: campaignId });
                } catch (dbError) {
                    logger.error('Failed to delete campaign from database', { 
                        error: dbError.message, 
                        campaignId: campaignId 
                    });
    ...
    async launchCampaign(req, res) {
        try {
            ...
            const workflowManager = require('../services/workflowManager');
            const profilePoolManager = require('../../profiles/services/profilePoolManager');
