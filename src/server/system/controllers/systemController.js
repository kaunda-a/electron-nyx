const profilePoolManager = require('../../domains/profiles/services/profilePoolManager');
const sessionStateManager = require('../services/sessionStateManager');
const itBrowserAPI = require('../../domains/profiles/services/itBrowserAPI');
const campaignManager = require('../../domains/campaigns/services/campaignManager');
...
const Profile = require('../../domains/profiles/models/Profile');
const { v4: uuidv4 } = require('uuid');

class SystemController {
    constructor() {
        this.proxyManager = profilePoolManager.proxyManager;
        this.dbProfileStorageService = dbProfileStorageService;
        this.dbProxyStorageService = dbProxyStorageService;
        this.systemStatus = {
            status: 'stopped',
            startTime: null,
            uptime: 0,
    ...
    async getAllProxies(req, res) {
        try {
            ...
            // If using database storage, also get proxies from SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (useDatabaseStorage) {
                const dbProxies = await this.dbProxyStorageService.listProxies();
                // Add database proxies to the response
                for (const dbProxy of dbProxies) {
                    // Check if this proxy is already in the in-memory list to avoid duplicates
                    const existingIndex = proxies.findIndex(p => p.id === dbProxy.id);
                    if (existingIndex === -1) {
                        const fullProxy = await this.dbProxyStorageService.loadProxy(dbProxy.id);
                        if (fullProxy) {
                            proxies.push({
                                ...fullProxy,
                                source: 'database'
                        ...
                        // Update the existing item with database data if it's newer
                        const dbProxyDetails = await this.dbProxyStorageService.loadProxy(dbProxy.id);
                        if (dbProxyDetails && dbProxyDetails.updatedAt && new Date(dbProxyDetails.updatedAt) > new Date(proxies[existingIndex].updatedAt || proxies[existingIndex].createdAt || 0)) {
    ...
    async createProxy(req, res) {
        try {
            ...
            // If using database storage, also save to SQLite
            const useDatabaseStorage = req.body?.useDatabaseStorage === true;
            if (useDatabaseStorage) {
                try {
                    await this.dbProxyStorageService.saveProxy(proxy.id, proxy.toJSON ? proxy.toJSON() : proxy);
                    logger.info('Proxy saved to database', { proxyId: proxy.id });
                } catch (dbError) {
                    logger.error('Failed to save proxy to database', { 
                        error: dbError.message, 
                        proxyId: proxy.id 
                    });
                    // Don't fail the entire request just because the database save failed
                }
            }
    ...
    async getProxy(req, res) {
        try {
            ...
            let proxy = this.proxyManager.getProxyById(proxyId);

            // If not found in memory and using database storage, check SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (!proxy && useDatabaseStorage) {
                const dbProxy = await this.dbProxyStorageService.loadProxy(proxyId);
                if (dbProxy) {
                    proxy = {
                        ...dbProxy,
                        id: proxyId,
                        source: 'database',
                        toJSON: function() { return this; } // Add toJSON for consistency
                    };
                }
            }
            ...
            res.json({
                success: true,
                data: proxy.toJSON ? proxy.toJSON() : proxy
            });
        } catch (error) {
    ...
    async deleteProxy(req, res) {
        try {
            ...
            // Also delete from database if using database storage
            const useDatabaseStorage = req.body?.useDatabaseStorage === true || req.query.useDatabaseStorage === 'true';
            if (useDatabaseStorage) {
                try {
                    await this.dbProxyStorageService.deleteProxy(proxyId);
                    logger.info('Proxy deleted from database', { proxyId: proxyId });
                } catch (dbError) {
                    logger.error('Failed to delete proxy from database', { 
                        error: dbError.message, 
                        proxyId: proxyId 
                    });
                    // Don't fail the request if only database deletion failed
                }
            }
    ...
    async getAllProfiles(req, res) {
        try {
            ...
            let profilesJson = profiles.map(profile => {
                console.log('Debug: Converting profile to JSON:', profile.id);
                return profile.toJSON ? profile.toJSON() : profile;
            });
            
            // If using database storage, also get profiles from SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (useDatabaseStorage) {
                const dbProfiles = await this.dbProfileStorageService.listProfiles();
                // Add database profiles to the response
                for (const dbProfile of dbProfiles) {
                    // Check if this profile is already in the in-memory list to avoid duplicates
                    const existingIndex = profilesJson.findIndex(p => p.id === dbProfile.id);
                    if (existingIndex === -1) {
                        const fullProfile = await this.dbProfileStorageService.loadProfile(dbProfile.id);
                        if (fullProfile) {
                            profilesJson.push({
                                ...fullProfile,
                                source: 'database'
                        ...
                        // Update the existing item with database data if it's newer
                        const dbProfileDetails = await this.dbProfileStorageService.loadProfile(dbProfile.id);
                        if (dbProfileDetails && dbProfileDetails.updatedAt && new Date(dbProfileDetails.updatedAt) > new Date(profilesJson[existingIndex].updatedAt || profilesJson[existingIndex].createdAt || 0)) {
                            profilesJson[existingIndex] = {
                                ...dbProfileDetails,
                                source: 'database'
                            };
                        }
    ...
    async getProfile(req, res) {
        try {
            ...
            let profile = profilePoolManager.getProfileById(profileId);

            // If not found in memory and using database storage, check SQLite
            const useDatabaseStorage = req.query.useDatabaseStorage === 'true' || req.body?.useDatabaseStorage === true;
            if (!profile && useDatabaseStorage) {
                const dbProfile = await this.dbProfileStorageService.loadProfile(profileId);
                if (dbProfile) {
                    profile = {
                        ...dbProfile,
                        id: profileId,
    ...
    async createProfile(req, res) {
        try {
            ...
            if (useDatabaseStorage) {
                try {
                    await this.dbProfileStorageService.saveProfile(profile.id, profile.toJSON ? profile.toJSON() : profile);
                    logger.info('Profile saved to database', { profileId: profile.id });
                } catch (dbError) {
    ...
    async deleteProfile(req, res) {
        try {
            ...
            if (useDatabaseStorage) {
                try {
                    await this.dbProfileStorageService.deleteProfile(profileId);
                    logger.info('Profile deleted from database', { profileId: profileId });
                } catch (dbError) {
                    logger.error('Failed to delete profile from database', { 
                        error: dbError.message, 
                        profileId: profileId 
                    });
                    // Don't fail the request if only database deletion failed
